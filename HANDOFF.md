...
> 갱신: 2026-07-07. 새 세션은 이 문서부터 읽고 이어가면 됩니다. **현재 작업 브랜치: `no_resv`** (예약 기능 전면 제거 — 아래 §0). 라이브 데모: https://hwangsi.github.io/hospital-agent/ (§0.1)
...
- **배포·검증 완료(2026-07-07)**: Pages 라이브에서 스냅샷 검색·미지원 안내·소개링크 Playwright e2e 통과.
- ⚠️ **스냅샷 h-index 폴백 오염 교훈(2026-07-07 해결)**: 첫 대량 생성 때 OpenAlex 429→회로차단(10분)로
  **교수급까지 617/807명이 pubmed-fallback**(예: 손태성 h=1)으로 저장됨. 조치 후 손태성 h=44(openalex).
  - 생성 스크립트에 **배치 모드 내장**: `OpenAlexClient._MIN_INTERVAL=0.3`(기본 0.12)·`_COOLDOWN=60`·enrich 세마포어 3.
  - 재생성 전 **L2 캐시 폴백 퍼지 필요**: `DELETE FROM hindex WHERE value LIKE '%pubmed-fallback%'` (.cache/hindex.sqlite3).
  - 잔여 폴백(~549명)은 영문명/저자엔티티 없는 전임의급(h≈0) — 구조적·정상. 교수급이 폴백이면 이상 신호.
...
- no_resv 최근: `114fea9`(스냅샷 h-index 교정+배치모드) `67f6d07`(Pages 정적데모) `d5dceb0`(예약 전면제거).
